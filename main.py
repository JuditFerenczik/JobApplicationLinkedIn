...
with open("output.txt", 'r') as out:
    ...
    checkedIds = [s_id.strip() for s_id in checkedIds]
...
for i in range(0, len(pagesButton)):
    ...
    pagesButton[i].click()
    time.sleep(2)
    jobsContainer = driver.find_elements(By.CSS_SELECTOR,
                                         " #main ul.scaffold-layout__list-container li.jobs-search-results__list-item")
    ...
    for j in range(0, len(jobsId)):
        if not (jobsId[j] in checkedIds):
            try:
                ...
                print(f"round {j}")
                # jobName = driver.find_elements(By.CSS_SELECTOR,
                # "#main ul.scaffold-layout__list-container li.jobs-search-results__list-item a.job-card-list__title")
